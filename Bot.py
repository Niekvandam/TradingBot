...
def wait_full_minute():
    ...
    print("waiting for {} seconds until {}...".format(sleeptime, datetime.utcnow().minute))
...
def long_or_short():
    ...
    if len(market_closes) > REQUIRED_CANDLES:
        if market_closes[-1] > market_opens[-1] > market_closes[-2] and long is False:
            print("GOING LONG! at price {} || {}".format(market_opens[-1], datetime.now()))
            long = True
        elif market_closes[-1] < market_opens[-1] < market_closes[-2] and long is True:
